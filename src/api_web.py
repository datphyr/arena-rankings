...
# ---------------------------------------------------------------------------
# Pipeline dashboard (optional, additive). Remove this block + pipeline.html
# and templates/pipeline to drop the dashboard without touching the pipeline.
# ---------------------------------------------------------------------------

def _pipeline_snapshot() -> dict:
    """Collect latest stage status + dead letters for the dashboard."""
    from .data_provider import Database
    from engine.status import read_latest_status

    db = Database()
    try:
        stages = read_latest_status(db)
        dead = db.client.execute(
            "SELECT post_id, stage, status, error, attempts, last_error "
            "FROM failed_posts FINAL ORDER BY last_error DESC LIMIT 50"
        )
        dead_letters = [
            {
                "post_id": r[0], "stage": r[1], "status": r[2],
                "error": r[3], "attempts": r[4],
                "last_error": _iso(r[5]),
            }
            for r in dead
        ]
        # Normalize datetimes to ISO strings so FastAPI can JSON-serialize.
        for s in stages:
            s["last_run_at"] = _iso_time(s.get("last_run_at"))
        return {"stages": stages, "dead_letters": dead_letters}
    finally:
        db.close()


def _iso_time(dt) -> str:
    """Return an ISO string for a datetime (or empty string if None)."""
    if dt is None:
        return ""
...
def pipeline_dashboard(request: Request):
    """Pipeline health dashboard: stage status, lag, backlog, dead letters."""
    ctx = _base_context(request, DataProvider())
    ctx["active"] = "pipeline"
    ctx["snapshot"] = _pipeline_snapshot()
    return templates.TemplateResponse(request, "pipeline.html", ctx)


@app.get("/api/pipeline/status")
def api_pipeline_status():
    """JSON snapshot for the dashboard's live-updating cards."""
    return JSONResponse(_pipeline_snapshot())


@app.post("/api/pipeline/retry/{post_id}")
def api_pipeline_retry(post_id: int):
    """Re-queue a dead-lettered post for reprocessing."""
    from engine.queue import PipelineQueue
    q = PipelineQueue(pending_status="discovered")
    try:
        ok = q.retry_dead_letter(post_id)
        return {"ok": ok, "post_id": post_id}
    finally:
        q.db.close()
