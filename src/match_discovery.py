...
extracts match IDs from the `listmatch` entries, and registers them in the
central `raw_posts` table with status 'discovered' (raw_html empty, sort_time
from the matchlist). The downloader then fetches each and flips it to
'downloaded'.

This is the discovery download mode: instead of scanning every post id
sequentially, we use the matchlist to find only the post IDs that are matches,
then download just those. sort_time (from the matchlist) is the reliable
chronological key — post_id is NOT chronological.

Resume strategy:
  1. Always start from page 1 and scan forward (newest first).
  2. On each page, extract match IDs.
  3. If all IDs on a page are already in raw_posts → we've caught up,
...
def discover_matches(
    ...
    """Discover matches from PlusForward matchlist into raw_posts (status 'discovered').
    ...
    try:
        existing_ids = db.raw_post_get_all_ids()
        known_count = len(existing_ids)
        last_known_page = db.get_last_known_page()
        initial_count = known_count
        logger.debug(f"raw_posts: {known_count} known, resume from page {last_known_page}")
        ...
        while True:
            ...
            if new_matches:
                added = db.register_discovered(new_matches, sort_times=new_timestamps)
                total_new += added
        ...
        final = db.raw_post_count_discovered()
        db.close()
        logger.debug(f"raw_posts: {final} discovered (was {initial_count} known)")
